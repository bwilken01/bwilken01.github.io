...
They want to understand if customers who did join the club have increased their spending the following three months.  The hypothesis is that, if customers are not paying for deliveries, they will be tempted to shop more frequently, and hopefully purchase more each time.
...
We applied Causal Impact Analysis (see full details below) using the *pycausalimpact* library in Python.
...
We used a 3-month pre-period for the algorithm to model and a 3-month post-period for the counterfactual.
...
We saw a 41.1% uplift in sales for those customers that joined the Delivery Club, over and above what we believe they would have spent, had the club not been created.  This was across the three month post-period, and the uplift was deemed to be significantly significant (@ 95%).
...
But to understand this, we really need to understand what *would have happened* had the event not taken place.

In most cases the trend *preceding the event in question* isn't stable, it is filled with ups and downs.  When some key event does take place, understanding what *would have happened had the event not taken place*, so we can estimate the true impact, can be difficult!
...
But there are a lot of cases where we just can’t run a randomized experiment, either because it’s expensive, or potentially it’s just impossible.  As an example, in the case of measuring the change in a share price after an event, we don’t really have a direct control group for comparison purposes.
...
* Manipulate the data specifically for the pycausalimpact algorithm
...
Just eyeing this first chart, it does indeed look like there is some increase in daily average spending for customers who joined the club, over-and-above what the model suggests they would have done if the club was never in existence.  We will look at the actual numbers for this very soon.
